...
function buildUserDataScript(githubRegistrationToken, label) {
  if (config.input.runnerHomeDir) {
    ...
    return [
      ...
      `./config.sh --url https://github.com/${config.githubContext.owner}/${config.githubContext.repo} --token ${githubRegistrationToken} --labels ${label} > ~/runner_build.log`,
      './run.sh >> ~/runner_build.log'
    ];
  } else {
    return [
      '#!/bin/bash',
      ...
      'curl -O -L https://github.com/actions/runner/releases/download/v2.313.0/actions-runner-linux-${RUNNER_ARCH}-2.313.0.tar.gz  > ~/runner_build.log',
      'tar xzf ./actions-runner-linux-${RUNNER_ARCH}-2.313.0.tar.gz  >> ~/runner_build.log',
      'export RUNNER_ALLOW_RUNASROOT=1',
      `./config.sh --url https://github.com/${config.githubContext.owner}/${config.githubContext.repo} --token ${githubRegistrationToken} --labels ${label} >> ~/runner_build.log`,
      './run.sh >> ~/runner_build.log'
    ];
